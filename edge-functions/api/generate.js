...
10. 输出必须是严格 JSON，不要 Markdown 代码块。`;
    ...
    const userPrompt = {
      ...
      largeLaunch: Boolean(body.largeLaunch),
      collabRanges: Array.isArray(body.collabRanges) ? body.collabRanges : [],
      collaborationRule: body.largeLaunch
        ? `这是大型发售/多群协作场景，需要输出一转九、可以转这条、回传确认等协作提醒。回传范围必须优先使用用户填写的范围：${(Array.isArray(body.collabRanges) && body.collabRanges.length ? body.collabRanges.join('、') : 'a1-a9')}。示例格式：✅ 范围 已同传完毕。`
        : '这不是大型发售/多群协作场景，禁止输出一转九、可以转这条、✅ a1-a9 已同传完毕等协作提醒。',
      transcript,
      outputSchema: {
        ...
        collabTips: body.largeLaunch ? ['1-3条多群协作提醒'] : []
      }
    };
